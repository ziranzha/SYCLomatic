...
/*
DPCT1050:6: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel1(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w) {
  for (int i = 0; i < w; ++i) {
    auto ret = surf.read_byte(i * sizeof(T));
    output[i] = ret;
  }
}
...
template <typename T, typename EleT>
/*
DPCT1050:7: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel1(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w, int h) {
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      auto ret = surf.read_byte(j * sizeof(T), i);
      output[w * i + j] = ret;
    }
  }
}
...
template <typename T, typename EleT>
/*
DPCT1050:8: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel1(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w, int h,
             int d) {
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < h; ++j) {
      for (int k = 0; k < w; ++k) {
        auto ret = surf.read_byte(k * sizeof(T), j, i);
...
template <typename T, typename EleT>
/*
DPCT1050:9: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel2(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w) {
  for (int i = 0; i < w; ++i) {
    auto ret = surf.read_byte(i * sizeof(T));
    output[2 * i] = ret.x();
    output[2 * i + 1] = ret.y();
...
template <typename T, typename EleT>
/*
DPCT1050:10: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel2(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w, int h) {
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      auto ret = surf.read_byte(j * sizeof(T), i);
...
template <typename T, typename EleT>
/*
DPCT1050:11: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel2(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w, int h,
             int d) {
  for (int i = 0; i < d; ++i) {
...
template <typename T, typename EleT>
/*
DPCT1050:12: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel4(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w) {
  for (int i = 0; i < w; ++i) {
    auto ret = surf.read_byte(i * sizeof(T));
    output[4 * i] = ret.x();
    output[4 * i + 1] = ret.y();
...
template <typename T, typename EleT>
/*
DPCT1050:13: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel4(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w, int h) {
  for (int i = 0; i < h; ++i) {
    for (int j = 0; j < w; ++j) {
      auto ret = surf.read_byte(j * sizeof(T), i);
...
template <typename T, typename EleT>
/*
DPCT1050:14: The template argument of the image_accessor_ext could not be deduced. You need to update this code.
*/
void kernel4(EleT *output, dpct::image_accessor_ext<T /*Fix the type manually*/, 1> surf, int w, int h,
             int d) {
  for (int i = 0; i < d; ++i) {
...
dpct::image_matrix *getInput(ArrT &expect, size_t w, const dpct::image_channel &desc) {
  dpct::image_matrix *input;
  input = new dpct::image_matrix(desc, sycl::range<2>(w, 0));
  dpct::dpct_memcpy(input->to_pitched_data(), sycl::id<3>(0, 0, 0), dpct::pitched_data(expect, sizeof(T) * w, sizeof(T) * w, 1), sycl::id<3>(0, 0, 0), sycl::range<3>(sizeof(T) * w, 1, 1));
  return input;
}

template <typename T, typename ArrT>
dpct::image_matrix *getInput(ArrT &expect, size_t w, size_t h,
                             const dpct::image_channel &desc) {
  dpct::image_matrix *input;
  input = new dpct::image_matrix(desc, sycl::range<2>(w, h));
  dpct::dpct_memcpy(input->to_pitched_data(), sycl::id<3>(0, 0, 0), dpct::pitched_data(expect, sizeof(T) * w, sizeof(T) * w, 1), sycl::id<3>(0, 0, 0), sycl::range<3>(sizeof(T) * w, h, 1));
  return input;
}

template <typename T, typename ArrT>
dpct::image_matrix *getInput(ArrT &expect, size_t w, size_t h, size_t d,
                             const dpct::image_channel &desc) {
  dpct::image_matrix *input;
  input = new dpct::image_matrix(desc, {w, h, d});
...
dpct::image_wrapper_base_p getSurf(dpct::image_matrix_p input) {
  dpct::image_data resDesc;
  memset(&resDesc, 0, sizeof(resDesc));

  resDesc.set_data(input);

  dpct::image_wrapper_base_p surf;
  surf = dpct::create_image_wrapper(resDesc);
...
int main() {
  ...
  { // 2 element char 1D image.
    ...
    /*
    DPCT1059:3: SYCL only supports 4-channel image format. Adjust the code.
    */
